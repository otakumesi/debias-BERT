...
def run():
    ...
    train_set = prepare_gap(datasets['train'])
    valid_set = prepare_gap(datasets['valid'])
    test_set = prepare_gap(datasets['test'])

    model = AutoModel.from_pretrained(model_args.model_name_or_path, config=config, cache_dir=model_args.cache_dir)
    ...
    train_loaders = {'train': PyTorchDataLoader(train_set, batch_size=train_args.train_batch_size),
                     'valid': PyTorchDataLoader(valid_set, batch_size=train_args.train_batch_size)}
    # runner.train(
    ...
    predictions = []
    test_loader = PyTorchDataLoader(test_set, batch_size=1)
    with open(OUTPUT_PATH / 'gep-system-output.tsv', 'w') as f:
        ...
        for i, preds in enumerate(runner.predict_loader(test_loader)):
            ...
            pred_labels = pref_probs.max(dim=-1).indeces
            ...
            for j, label in enumerate(pred_labels):
                ...
                writer.writerow([test_row['ID'], test_row['Pronoun'], a_coref, b_coref)
